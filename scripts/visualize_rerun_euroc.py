    ...
    with open(tum_path, "r") as f:
        for line in f:
            ...
            if len(parts) >= 8:
                ...
                q = [float(parts[7]), float(parts[4]), float(parts[5]), float(parts[6])] # (qw, qx, qy, qz)
...
def align_trajectories(gt_pos, est_pos):
    """Align first pose of estimated trajectory to ground truth origin."""
    if len(est_pos) == 0 or len(gt_pos) == 0:
        return est_pos
    offset = gt_pos[0] - est_pos[0]
    return est_pos + offset
...
def main():
    ...
    # Align estimated trajectory to ground truth initial coordinate frame
    if len(est_pos) > 0 and len(gt_pos) > 0:
        est_pos_aligned = align_trajectories(gt_pos, est_pos)
    else:
    ...
    # Log complete static Ground Truth 3D path for reference
    if len(gt_pos) > 0:
        rr.log(
    ...
    # Stream frames and live 3D poses synchronously
    ...
    for f_idx, (ts_sec, img_path) in enumerate(cam_data):
        ...
        if len(gt_pos) > gt_idx:
            ...
            accumulated_gt_path.append(curr_gt_p)
        ...
        if len(est_pos_aligned) > est_idx:
            ...
            if len(accumulated_est_path) > 1:
                rr.log(
                    "world/neurovio/trajectory_strip",
                    rr.LineStrips3D([np.array(accumulated_est_path)], colors=[[0, 200, 255]], radii=0.04)
                )

        # Log error metric
        if len(gt_pos) > gt_idx and len(est_pos_aligned) > est_idx:
